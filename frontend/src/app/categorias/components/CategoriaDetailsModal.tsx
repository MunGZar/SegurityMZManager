...
import { Layers, Calendar, ShoppingBag } from 'lucide-react';
...
export function CategoriaDetailsModal({
  open,
  onOpenChange,
  categoria,
}: CategoriaDetailsModalProps) {
      ...
      <DialogContent className="sm:max-w-[500px] border-zinc-800 bg-zinc-900 text-zinc-100">
        <DialogHeader>
          ...
          <DialogDescription className="text-zinc-400">
            Detalles y clasificación del tipo de producto o servicio.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5 py-3">
          ...
          <div className="flex gap-4 items-start p-4 rounded-lg bg-zinc-950/60 border border-zinc-800/60">
            ...
            <div className="overflow-hidden flex-1">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h3 className="text-lg font-bold text-white leading-tight truncate">{categoria.nombre}</h3>
                {categoria.deletedAt ? (
              ...
              <p className="text-xs text-zinc-500 mt-1">ID: {categoria.id}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <p className="text-sm text-zinc-200 font-medium flex items-center gap-1.5">
                ...
                {formatDate(categoria.createdAt, true)}
              </p>
            </div>
            <div className="space-y-1">
              ...
              <p className="text-sm text-zinc-200 font-medium flex items-center gap-1.5">
                <Calendar className="h-3.5 w-3.5 text-zinc-500" />
                {formatDate(categoria.updatedAt, true)}
              </p>
            </div>
          </div>
          ...
          <div className="border-t border-zinc-800/60 pt-4 space-y-3">
            ...
            <div className="p-3 rounded-lg bg-zinc-950/30 border border-zinc-800/50 flex items-center gap-2">
              ...
              <div>
                <p className="text-xs text-zinc-500 font-medium">Productos de esta categoría</p>
