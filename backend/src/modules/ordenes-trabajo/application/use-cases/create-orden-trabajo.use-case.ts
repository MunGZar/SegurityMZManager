  ...
  async execute(dto: CreateOrdenTrabajoDto): Promise<OrdenTrabajoCompleta> {
    ...
    if (!cotizacion) {
      throw new NotFoundException(
        `La cotización con ID '${dto.cotizacionId}' no existe.`,
      );
    }
    ...
    const existingOrder = await this.ordenesRepository.findByCotizacionId(
      dto.cotizacionId,
    );
    if (existingOrder) {
    ...
    const direccion =
      dto.direccion || cotizacion.cliente?.direccion || undefined;
